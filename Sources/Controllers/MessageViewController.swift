...
public final class MessageViewController: UIViewController {
    ...
    /// Border view.
    public private(set) lazy var imageBorderView: UIView = self.makeBorderView()
    ...
    private func animate(borderViewAngle: CGFloat) {
        guard status.state == .processing else {
            imageBorderView.transform = .identity
            return
        }
        ...
        UIView.animate(
            ...
            options: [.beginFromCurrentState],
            animations: ({ [weak self] in
                self?.imageBorderView.transform = CGAffineTransform(rotationAngle: borderViewAngle)
            }),
            completion: ({ [weak self] _ in
    ...
    private func handleStatusUpdate() {
        ...
        imageBorderView.layer.removeAllAnimations()
...
extension MessageViewController {
    private func makeExpandedConstraints() -> [NSLayoutConstraint] {
        ...
        return [
            ...
            imageBorderView.topAnchor.constraint(equalTo: imageView.topAnchor, constant: -12),
            imageBorderView.centerXAnchor.constraint(equalTo: blurView.centerXAnchor),
            imageBorderView.widthAnchor.constraint(equalToConstant: borderSize),
            imageBorderView.heightAnchor.constraint(equalToConstant: borderSize)
        ]
    }
